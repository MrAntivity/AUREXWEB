...
export default function ReportsPage() {
  ...
  const canView = user?.role === "super_admin" || user?.role === "finance_viewer";
  ...
  const topProducts = useMemo(() => {
    ...
    for (const o of orders) {
      for (const item of o.items) {
        const e = map[item.product.sku] ?? { name: item.product.name, qty: 0, spend: 0 };
        e.qty += item.qty;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Spend by category */}
        <div className="card">
          <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Spend by Category</h2>
        ...
        <div className="card">
          <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Order Status Breakdown</h2>
          {orders.length === 0 ? (
            <p className="py-8 text-center text-sm text-gray-400">No orders submitted yet.</p>
          ) : (
            <div className="space-y-4">
              ...
              <div className="mt-2 border-t border-gray-100 pt-3 text-xs text-gray-400 dark:border-white/8 dark:text-gray-500">
      ...
      {topProducts.length > 0 && (
        <div className="card">
          <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Top Ordered Products</h2>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {topProducts.map((p, i) => (
                  <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-white/5">
                    ...
                    <td className="py-2.5 text-gray-600 dark:text-gray-300">{p.qty}</td>
                    <td className="py-2.5 font-semibold text-gray-900 dark:text-white">${p.spend.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
