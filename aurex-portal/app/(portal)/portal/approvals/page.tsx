...
function ApprovalRow({
  ...
  return (
    <>
      <tr className="cursor-pointer hover:bg-gray-50/60" onClick={() => setOpen((p) => !p)}>
        ...
        <td className="py-3 text-sm font-medium text-gray-900">{order.requester.name}</td>
        <td className="py-3 text-sm text-gray-500">{order.requester.department}</td>
            ...
            <div className="space-y-4 rounded-lg border border-gray-100 bg-gray-50 p-4">
              ...
              <div className="space-y-3 border-t border-gray-200 pt-3">
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-700 placeholder-gray-400 focus:border-aurex-blue focus:outline-none focus:ring-1 focus:ring-aurex-blue"
                />
...
function ReviewedRow({ order }: { order: Order }) {
  ...
  return (
    <>
      <tr
        className="cursor-pointer opacity-70 hover:bg-gray-50/60"
        onClick={() => setOpen((p) => !p)}
      >
        <td className="py-3 pr-3">
          {open ? (
        ...
        <td className="py-3">
          <span
            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${cfg.cls}`}
          >
            {cfg.label}
          </span>
        </td>
...
export default function ApprovalsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {reviewed.length > 0 && (
        <div className="card">
          <h2 className="mb-4 text-sm font-semibold text-gray-900">Previously Reviewed</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
