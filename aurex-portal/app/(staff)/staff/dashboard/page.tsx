  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        {[
          { label: "Total Products",  value: String(totalProducts), icon: Package,       color: "bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400",     href: "/staff/products" },
          { label: "Pending Orders",  value: String(pendingOrders), icon: Clock,         color: "bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400", href: "/staff/orders" },
          { label: "Low Stock Items", value: String(lowStock),      icon: AlertTriangle, color: "bg-orange-50 text-orange-500 dark:bg-orange-500/10 dark:text-orange-400", href: "/staff/products" },
          { label: "Out of Stock",    value: String(outOfStock),    icon: ShoppingBag,   color: "bg-red-50 text-red-500 dark:bg-red-500/10 dark:text-red-400",          href: "/staff/products" },
        ].map((s) => (
          <Link
            key={s.label}
            href={s.href}
            className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-white/10 dark:bg-[#1a1a2a]"
          >
      ...
      {(lowStock > 0 || outOfStock > 0) && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
          <AlertTriangle className="mt-0.5 shrink-0 text-amber-600 dark:text-amber-400" size={16} />
          <div>
            ...
            <Link href="/staff/products" className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-amber-700 hover:underline dark:text-amber-400">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Recent Orders */}
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1a1a2a]">
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5">
              {recentOrders.map((o) => (
                <div key={o.id} className="flex items-center justify-between py-2.5">
                  <div>
                    <p className="text-xs font-semibold text-gray-900 dark:text-white">{o.requestNumber}</p>
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5">
              {recentAudit.map((entry) => (
                <div key={entry.id} className="flex items-start gap-3 py-2.5">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-aurex-blue/10 text-[10px] font-bold text-aurex-blue dark:bg-aurex-blue/15">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-400">
                      {entry.userName} · {new Date(entry.timestamp).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                    </p>
                  </div>
                </div>
              ))}
