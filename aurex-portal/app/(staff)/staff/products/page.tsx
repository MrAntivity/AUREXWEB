...
import { Search, Plus, X, ImageIcon, Save, AlertTriangle, ChevronDown } from "lucide-react";
...
export default function StaffProductsPage() {
  ...
  function stockBadge(stock: number) {
    if (stock === 0) return <span className="rounded-full bg-red-50 px-2 py-0.5 text-xs font-medium text-red-600 ring-1 ring-inset ring-red-200">Out of stock</span>;
    if (stock < 10) return <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-200">{stock} left</span>;
    return <span className="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-200">{stock} in stock</span>;
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">{products.length} products total</p>
        </div>
        <button
          onClick={() => setIsAdding(true)}
          className="flex items-center gap-2 rounded-lg bg-aurex-blue px-4 py-2 text-sm font-semibold text-white hover:bg-aurex-blue-light transition-colors"
        >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-lg border border-gray-200 bg-white py-2 pl-3 pr-8 text-sm text-gray-700 focus:border-aurex-blue focus:outline-none"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((p) => (
                <tr key={p.sku} className="group hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{p.fulfillmentTime}</td>
                  <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">
                    {new Date(p.updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                    <span className="ml-1 text-gray-300">by {p.updatedBy}</span>
                  </td>
                  <td className="px-4 py-3">
                    <button
                      onClick={() => setEditProduct({ ...p })}
                      className="rounded-md border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-600 opacity-0 transition group-hover:opacity-100 hover:border-aurex-blue hover:text-aurex-blue"
                    >
                      Edit
                    </button>
                  </td>
...
function ProductModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-100 bg-white shadow-2xl"
      ...
      >
        ...
        <div className="space-y-5 p-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="flex aspect-square items-center justify-center rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 text-center">
                  <div className="p-2">
            ...
            <p className="mt-2 text-xs text-gray-400">Image uploads will be available when cloud storage is connected.</p>
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <F label="Product Name" required>
              <input className="field-input" value={product.name} onChange={(e) => onChange({ name: e.target.value })} placeholder="e.g. BD Luer-Lock Syringe 5mL" />
            </F>
            <F label="SKU" required>
              <input className="field-input" value={product.sku} onChange={(e) => onChange({ sku: e.target.value })} placeholder="e.g. SYR-003" disabled={isEdit} />
            </F>
            <F label="Category">
              <select className="field-input" value={product.category} onChange={(e) => onChange({ category: e.target.value as Category })}>
                {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </F>
            <F label="Unit">
              <input className="field-input" value={product.unit} onChange={(e) => onChange({ unit: e.target.value })} placeholder="e.g. Box/50, Each, Pack/100" />
            </F>
            <F label="Price ($)">
              <input className="field-input" type="number" step="0.01" min="0" value={product.price} onChange={(e) => onChange({ price: parseFloat(e.target.value) || 0 })} />
            </F>
            <F label="Units in Stock">
              <input className="field-input" type="number" min="0" value={product.stock} onChange={(e) => onChange({ stock: parseInt(e.target.value) || 0 })} />
            </F>
            <F label="Fulfillment Time">
              <input className="field-input" value={product.fulfillmentTime} onChange={(e) => onChange({ fulfillmentTime: e.target.value })} placeholder="e.g. Same day, 1–2 business days" />
            </F>
            <F label="Estimated Delivery">
              <input className="field-input" value={product.estimatedDelivery} onChange={(e) => onChange({ estimatedDelivery: e.target.value })} placeholder="e.g. 2–4 business days" />
            </F>
          </div>
          ...
          {product.stock === 0 && (
            <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5">
              ...
              <p className="text-xs font-medium text-red-600">Stock is 0 — this product will show as &quot;Out of stock&quot; to users.</p>
            </div>
          )}
        </div>
        ...
        <div className="sticky bottom-0 flex justify-end gap-3 border-t border-gray-100 bg-white px-6 py-4">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-aurex-blue px-5 py-2 text-sm font-semibold text-white hover:bg-aurex-blue-light disabled:opacity-50 transition-colors"
          >
      ...
      <style jsx>{`
        ...
        .field-input:focus {
          border-color: rgb(var(--color-aurex-blue));
          box-shadow: 0 0 0 2px rgb(var(--color-aurex-blue) / 0.15);
        }
